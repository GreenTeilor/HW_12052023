    ...
    <c:forEach items="${categoryProducts}" var="product">
        <div class="col d-flex justify-content-center">
            <div class="card" style="width: 22rem; margin: 20px; background-color: lightgrey">
                <a href="product?id=${product.id()}"><img src="${product.imagePath()}" class="card-img-top" style="height: 25rem;" alt="..."></a>
                <div class="card-body" style="text-align: center">
                    <h2 class="card-title">${product.name()}</h2>
                    <p class="card-text">Цена: <fmt:formatNumber value="${product.price()}"
                                                                 type="currency"/><br>
                        ${product.description()}</p>
                    <a href="product?id=${product.id()}" class="btn btn-primary">Посмотреть</a>
                </div>
            </div>
        </div>
    </c:forEach>
